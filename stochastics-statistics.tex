...
\subsubsection{Cumulative Discrete Distribution Functions}
...
&= P\left(X \leq x, Y \leq y\right) - P\left(X \leq x, Y \leq y - 1\right) - P\left(X \leq x - 1, Y \leq y\right) \\
&+ P\left(X \leq x - 1, Y \leq y - 1\right) \\
...
\end{align}
Here, $Q$ is treated as the approximating distribution. The KL divergence is defined only for when $Q\left(x_{i}\right) = 0$ implies $P\left(x_{i}\right) = 0$ for all $i$. A property of the KL divergence is always non-negative, ie. $\operatorname{KL}\left(P\|Q\right) \geq 0$.
\begin{proof}
\begin{align}
...
\subsubsection{Symmetrised KL Divergence}
...
Let $X_{1}, X_{2}, \dots$ be real valued random vectors and let $\left\{X_{1}, X_{2}, \dots \right\}$ be a sequence of random vectors. The sequence is said to converge in distribution to a random variable $X$ if
\begin{equation}
