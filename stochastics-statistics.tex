...
\end{gather}
Then using the definition of the correlation coefficient:
\begin{equation}
...
An estimator $\hat{\theta}$ for $\theta$ is said to be consistent if loosely speaking, the estimate $\hat{\theta}$ approaches $\theta$ as the sample size $n$ grows large.
...
\begin{equation}
\mathbf{C} = \mathbb{E}\left[\left(\mathbf{X} - \bar{\mathbf{X}}\right)\left(\mathbf{X} - \bar{\mathbf{X}}\right)^{\top}\right]
\end{equation}
...
\begin{align}
\mathbf{u}^{\top}\mathbf{C}\mathbf{u} &= \mathbf{u}^{\top}\mathbb{E}\left[\left(\mathbf{X} - \bar{\mathbf{X}}\right)\left(\mathbf{X} - \bar{\mathbf{X}}\right)^{\top}\right]\mathbf{u} \\
...
\begin{align}
\mathbf{u}^{\top}\mathbf{C}\mathbf{u} &= \mathbb{E}\left[ss^{\top}\right] \\
...
The ratio distribution of random variables $X$ and $Y$ is the distribution of $W = \dfrac{X}{Y}$. If $X$ and $Y$ are independent and have probability density functions $f_{X}\left(x\right)$ and $f_{Y}\left(y\right)$ respectively, then
\begin{equation}
...
The mean of the ratio distribution is not as straightforward, but can be approximated using Taylor expansions.
...
\section{Maximum Spacing Estimation}
...
\end{align}
Introduce the change of variables $\tau = t - t'$, noting that the integrand only depends on $\tau$. To determine the limits of integration, we reason that if $-T\leq t \leq T$ and $-T \leq t'\leq T$, then $-2T \leq \tau \leq 2T$. Then a graphical sketch on the $t$-$t'$ plane of the region $\Omega$ for $t'$ given by the intersection
\begin{equation}
...
\subsection{Spectral Factorisation Theorem}
