...
\subsubsection{Sampling Distribution of the Normal Sample Mean}
...
\end{equation}
Because $X$ and $Y$ are continuous, this can be stated as the original inequality.
\end{proof}
...
\begin{equation}
z_{k} = Cx_{k} + Du_{k} + Fv_{k}
\end{equation}
where the measured output $z_{k} \in \mathbb{R}^{p}$ is known and the noise is of dimension $v_{k} \in \mathbb{R}^{d'}$, and matrices $C$, $D$, $F$ are of appropriate dimension. We again assume that $v_{k}$ is independent zero-mean unit variance white noise with $\operatorname{Cov}\left(Fv_{k}\right) = FF^{\top}$, or alternatively we can let $R = FF^{\top}$ and write the process as
\begin{equation}
z_{k} = Cx_{k} + Du_{k} + v_{k}'
\end{equation}
...
\begin{gather}
\hat{x}_{k + 1} = \hat{x}_{k + 1}^{-} + K_{k + 1}\left(z_{k + 1} - C\hat{x}_{k + 1}^{-} - Du_{k}\right) \\
P_{k + 1} = \left(I - K_{k + 1}C\right)P_{k + 1}^{-}
\end{gather}
\end{enumerate}
If noises $w_{k}'$ and $v_{k}'$ are Gaussian, then the posterior estimates specify the full (Gaussian) posterior distribution. Note that the algorithm is also applicable to a time-varying system with known time-varying matrices $A_{k}, B_{k}, Q_{k}, C_{k}, D_{k}, R_{k}$.
...
\begin{align}
\mathbb{E}\left[\hat{x}_{k + 1}\right] &= \mathbb{E}\left[\hat{x}_{k + 1}^{-} + K_{k + 1}\left(z_{k + 1} - C\hat{x}_{k + 1}^{-}\right) - Du_{k}\right] \\
&= \mathbb{E}\left[\hat{x}_{k + 1}^{-} + K_{k + 1}\left(Cx_{k + 1} + Du_{k} + v_{k + 1}' - C\hat{x}_{k + 1}^{-}\right) - Du_{k}\right] \\
...
\begin{align}
P_{k}^{-} &= \mathbb{E}\left[e_{k}^{-}\left(e_{k}^{-}\right)^{\top}\right] \\
&= \mathbb{E}\left[\left(x_{k} - \hat{x}_{k}^{-}\right)\left(x_{k} - \hat{x}_{k}^{-}\right)^{\top}\right]
\end{align}
For the posterior error covariance matrix, we can rewrite this using $x_{k} - \hat{x}_{k} = \left(x_{k} - \hat{x}_{k}^{-}\right) - \left(\hat{x}_{k} - \hat{x}_{k}^{-}\right)$ to give
\begin{align}
...
\begin{equation}
\hat{x}_{k} = \hat{x}_{k}^{-} + K_{k}\left(z_{k} - C\hat{x}_{k}^{-} - Du_{k - 1}\right)
\end{equation}
which can be rearranged so that
\begin{align}
\hat{x}_{k} - \hat{x}_{k}^{-}  &= K_{k}\left(z_{k} - C\hat{x}_{k}^{-} - Du_{k - 1}\right) \\
&= K_{k}\left(Cx_{k} + Du_{k - 1} + v_{k}' - C\hat{x}_{k}^{-} - Du_{k - 1}\right) \\
